...
fn brute() -> Result<(i64, Vec<u64>), String> {
    let mut bv: Vec<u64> = vec![];
...
pub fn day2_part1() -> Result<i64, String> {
    ...
    for v in vals {
        ...
        for length in (start_digits..=end_digits).step_by(2) {
            ...
            let dif = end - start + 1;
            sum += start * conv * (dif*(dif+1)/2);
            v += start * conv * (dif*(dif+1)/2);
        ...
        if v != bv[i] {
            println!("Mismatch at {i}: real {} vs false {}", bv[i], v);
        }
        i+=1;
    }
